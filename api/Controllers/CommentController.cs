...
public class CommentController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateComment(CommentsDto commentsDto, int IdeaId)
    {
        try
        {
            ...
            //Fetch user info
            var userEmail = User.FindFirstValue(api.Constants.AuthConstants.EmailClaim)
    ...
    {
        //Fetch user
        var userEmail = User.FindFirstValue(api.Constants.AuthConstants.EmailClaim) ?? User.FindFirstValue(ClaimTypes.Email) ?? "Email not found";
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
        if (string.IsNullOrWhiteSpace(userId))
        {
        ...
        if (comments.Count == 0)
        {
            _logger.LogInformation("No comments found in Idea: {ideaTitle}", idea.Title);
