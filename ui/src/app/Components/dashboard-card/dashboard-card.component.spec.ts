...
@Component({
  template: `
    <app-dashboard-card [title]="'My Widget'" [variant]="'dark-mode'" [icon]="'heroBell'">
...
describe('DashboardCardComponent', () => {
  ...
  describe('Inputs and Directives Bindings', () => {
    it('should display the correct card header title', () => {
      const headerTitleSpan = hostFixture.debugElement.query(By.css('.card-header span:not(.icon)'));
      expect(headerTitleSpan).toBeTruthy();
      expect(headerTitleSpan.nativeElement.textContent.trim()).toBe('My Widget');
    });
    ...
    it('should apply the matching CSS variant class to the parent card container', () => {
      ...
      expect(cardContainerEl.nativeElement.classList.contains('dark-mode')).toBeTrue();
    });
  ...
  describe('Content Projection', () => {
    it('should render the projected elements inside the card-content container', () => {
      const cardContentEl = hostFixture.debugElement.query(By.css('.card-content'));
      expect(cardContentEl).toBeTruthy();

      const projectedEl = hostFixture.debugElement.query(By.css('#projected-widget'));
      expect(projectedEl).toBeTruthy();
      expect(projectedEl.nativeElement.textContent.trim()).toBe('Projected Dashboard Metric');
    });
  });
});
